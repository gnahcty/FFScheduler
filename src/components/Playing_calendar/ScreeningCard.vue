<template>
  <div v-for="film in props.films" :key="film.filmId">
    <div
      class="card absolute flex w-full flex-col justify-between rounded-md border border-emerald-200 p-2 text-emerald-800 hover:bg-emerald-100"
      :style="computeCardStyle(film)"
    >
      <router-link to="/details">
        <div class="ellipsis flex w-full flex-col overflow-clip">
          <span class="text-sm opacity-75"
            >{{ findTime(film) }} - {{ findEndingTime(film) }}</span
          >
          <span>{{ film.CName }}</span>
          <span class="text-pretty text-sm opacity-75">{{ film.EName }}</span>
        </div>
      </router-link>
      <span v-if="like" class="pi pi-heart-fill text-right" @click="like = !like"></span>
      <span v-else class="pi pi-heart text-right" @click="like = !like"></span>
    </div>
  </div>
</template>
